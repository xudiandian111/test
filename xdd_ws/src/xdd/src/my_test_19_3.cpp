...
#define pi 3.1415926
#define pixel_min  0              //用于判断是否还能检测到黑圆中心
using namespace std;
...
double PXFLOW_beginpose_x = 0.0;
double PXFLOW_beginpose_y = 0.0;
double aim_x_pxflow = 0.0;
double aim_y_pxflow = 0.0;
...
bool flag_getbegin_pxflow = false;
...
void vision_pose_cb(const geometry_msgs::Pose2D::ConstPtr& msg)
{
    ...
    //***********************切换xy数据来源*************************
    if(msg->x < pixel_min || msg->y < pixel_min )
    {
...
void judge_can_detect(const geometry_msgs::PoseStamped::ConstPtr& msg) 
{
    ...
    if(high_stable_time >= 20)
    {
        ...
        {   
            ...
            if(pose_hold_time >= 5)  //在pose回调里已经判断了是否回到初始位置已经稳定，但是第一次获得aim_num还是会有问题
            {
...
{   
    ...
    //在前往目标点的过程中丢失了xy位置，坐标系问题local_ned 转成body_ned
	if(flag_use_data_from == PXFLOW)
	{		
		if(!flag_getbegin_pxflow)
		{
			ROS_INFO("Using PX4FLOW!!!!!!!!!!!!!!!!!!!!!!!");

			x.set_beginpose(x.read_beginpose());               // 将图像的初始的位置设置为px4flow的初始的位置
            y.set_beginpose(y.read_beginpose());
	
			x.init(AllSetPositionX[aim_num % 4], 0.2, 0, 0.0);
            y.init(AllSetPositionY[aim_num % 4], 0.2, 0, 0.0);  //设置目标点
			...
			flag_getbegin_pxflow = true;
		}

		if(flag_start == 1 && flag_getbegin_pxflow)
		{
            x.set_curpose((msg->pose.position.x - x_body.read_beginpose()) * cos(cur_yaw.data)
                        + (msg->pose.position.y - y_body.read_beginpose()) * sin(cur_yaw.data));
            y.set_curpose( (-1) * (msg->pose.position.x - x_body.read_beginpose()) * sin(cur_yaw.data)
