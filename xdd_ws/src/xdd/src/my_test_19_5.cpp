...
int flag_use_data_from = PXFLOW;
bool flag_change_use_data = false;
...
{   
    ...
    // 已经检测带气球下落之后，再调用sevcice,这时候已经有aim_num;

    cout << "aim_pose_num:" << req.aim_num << endl;
    // cout << "response:" << res.is_back_home << endl;
    // cout<<"flag_can_get_aim:"<< flag_can_get_aim <<endl;
    
    // if(flag_can_start_detect > 0.99)   
    // if(flag_can_get_aim)
    // cout<<flag_can_start_detect.data<<endl;

    if(flag_can_start_detect.data > 0.90)      
    {   
        // client_call = true;
        cout << "进去了!!!!!!!!!!!!!!!!!!!!!!!!!!" << endl;
...
{
//****************************************************************************
    // if(msg->x <= 0 && msg->y <= 0)     //检测不到图像坐标信息
    // {
    //     flag_use_data_from = PXFLOW;
    //     flag_change_use_data = true;
    // }

    // if(msg->x > 0 && msg->x > 0)
    // {
    //     flag_use_data_from = PICTURE;
    //     flag_change_use_data = false;
    // }
...
//****************************************************************************
	...
	{   
		...
		{   
            ...
            // if(z_hold_time > 150)                           //稳定之后，才设定xy目标点位置  //切换到这里，Z必然已经稳定了
            {     
                 if(flag_have_got_aim)                     //首先需要已经获得xy目标点位置aim_num
                 {
                    ...
                    // 飞一个轮回
                    // 用于判断飞机是否到达目标点
                    if(fabs(x.read_err()) <= 0.25 && fabs(y.read_err()) <= 0.25)
                    {
...
{   
    ...
    {    
        ...
        x_px4.set_beginpose(msg->pose.position.x);
        y_px4.set_beginpose(msg->pose.position.y);
    
        x_px4.init(0.0, 0.5, 0, 0.1);
        y_px4.init(0.0, 0.5, 0, 0.1);
    
        flag_getbegin_pos = 1;                
    }


    if(flag_use_data_from == PXFLOW)                                        //第一次实在ned坐标系下运动
    { 
        if(flag_start == 1 && flag_getbegin_pos == 1)
        {   
            //BODY_NED坐标系的区别，将LOCAL_NED坐标转换到BODY_NED
            x_px4.set_curpose(x_px4.read_beginpose() + (msg->pose.position.x - x_px4.read_beginpose()) * cos(cur_yaw.data)
                            + (msg->pose.position.y - y_px4.read_beginpose()) * sin(cur_yaw.data));
            y_px4.set_curpose(y_px4.read_beginpose() + (-1)*(msg->pose.position.x - x_px4.read_beginpose()) * sin(cur_yaw.data)
                            + (msg->pose.position.y - y_px4.read_beginpose()) * cos(cur_yaw.data));   
                   
            double AllSetPositionX[] = {1, 0, -1, 0};
            double AllSetPositionY[] = {0, 1, 0, -1};
            
            // ！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！
            // if(flag_change_use_data)                       //如果之前的数据是图像坐标系下，现在切换到机体坐标系下
            {     
                 if(flag_have_got_aim)                     //首先需要已经获得xy目标点位置aim_num
                 {
                    x_px4.set_aimpose(x_px4.read_beginpose() + AllSetPositionX[aim_num % 4]);
                    y_px4.set_aimpose(y_px4.read_beginpose() + AllSetPositionY[aim_num % 4]);
                    
                    // ×××××××××××××××××××××××××××××××××××××××××××××××××××××××
                    // 飞一个轮回
                    // 用于判断飞机是否到达目标点
                    if(fabs(x_px4.read_err()) <= 0.15 && fabs(y_px4.read_err()) <= 0.15)
                    {
                        hold_time++;                                 //到达目标点的稳定时间
                    }
                    if(hold_time > 150)
                    {
                        x_px4.set_aimpose(x_px4.read_beginpose());          //设定新的目标点为初始位置，返回起点
                        y_px4.set_aimpose(y_px4.read_beginpose()); 
                    ...
                    {   
                        ...
                        // flag_getbegin_pos = 0;
                        // flag_getbegin_picture = 0;                  //每次重回黑圆下方之后，重置初始坐标点，
                        // flag_use_data_from = PICTURE;                   //切回到图像坐标系下再运动
                    }
                    // ×××××××××××××××××××××××××××××××××××××××××××××××××××××××××     
                }  
                xdd_xy_pub.x = x_px4.read_curpose() - x_px4.read_beginpose();
                xdd_xy_pub.y = y_px4.read_curpose() - y_px4.read_beginpose(); 
                error_xy.x = x_px4.read_err();
                error_xy.y = y_px4.read_err(); 
            } 		        
        }

        if(fabs(z.read_err()) < 0.15  && fabs(x_px4.read_err()) < 0.15 && fabs(y_px4.read_err()) < 0.15)
        {            
            px4flow_hold_time++;           
        }
        ...
        cout << "px4flow_hold_time:" << px4flow_hold_time << endl;
        ROS_INFO("px4flow_x:[%lf]", x_px4.read_curpose());
        ROS_INFO("px4flow_x_aimpose:[%lf]", x_px4.read_aimpose());
        ROS_INFO("px4flow_read_err:[%lf]", x_px4.read_err());
        ROS_INFO("px4_set_X_vel:[%lf]", x_px4.out());
        ROS_INFO("px4flow_y:[%lf]", y_px4.read_curpose());
        ROS_INFO("px4flow_y_aimpose:[%lf]", y_px4.read_aimpose());
        ROS_INFO("px4_set_Y_vel:[%lf]", y_px4.out());  
        ROS_INFO("px4flow_read_err:[%lf]", y_px4.read_err());
        ...
        x_px4.update();
        y_px4.update();
        set_LinearVel_XY(vel, x_px4.out(), y_px4.out());
