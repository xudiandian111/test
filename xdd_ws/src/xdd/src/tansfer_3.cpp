...
// bool is_back_home_cb(xdd::get_aim::Request &req, xdd::get_aim::Response &res)
// {   
    ...
    {   
        ...
        {   
            ...
            {     
                 ...
                 {
                    flag_can_start_detect = false;           //这时必定已经接收到目标点信息，修改检测为0.0,false
                    ...
                    {   
                        ...
                        {                           
                            ...
                            can_start_detect_pub.publish(msg_bool);     //发布能否开始检测的命令
...
//无论如何先初始化
    ...
    if(flag_use_data_from == PXFLOW)
    {
        ...
        {   
            ...
            // cout << "flag_can_start_detect" << flag_can_start_detect << endl;
            cout << "aim_pose_num:" <<aim_num << endl;

            ROS_INFO("px4flow_x_begin:[%lf]", px4_x.read_beginpose());
            ROS_INFO("px4flow_x:[%lf]", px4_x.read_curpose());
            ROS_INFO("px4flow_x_aimpose:[%lf]", px4_x.read_aimpose());
            ROS_INFO("px4_set_x_vel:[%lf]", px4_x.out());

            ROS_INFO("px4flow_y_begin:[%lf]", px4_y.read_beginpose());
            ROS_INFO("px4flow_y:[%lf]", px4_y.read_curpose());
            ROS_INFO("px4flow_y_aimpose:[%lf]", px4_y.read_aimpose());
            ROS_INFO("px4_set_y_vel:[%lf]", px4_y.out());  

            ROS_INFO("px4flow_z_begin:[%lf]", z.read_beginpose());
            ROS_INFO("z.read_aimpose:[%lf]", z.read_aimpose());
            ROS_INFO("z.read_curpose:[%lf]", z.read_curpose());
...
int main(int argc, char **argv)
{
    ...
    // ros::ServiceServer get_aim_num_service = nh.advertiseService<xdd::TF>
    //         (is_back_home_cb);  
