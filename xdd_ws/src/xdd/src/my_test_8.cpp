...
{   
   ...
   {    
        ...
        x.init(x.read_beginpose() + 0.0, 2.0, 0, 0.3);
        y.init(y.read_beginpose() + 0.0, 2.0, 0, 0.3);
    ...
    {     
            // if(flag_go_back_home  == 0)
            {
                ...
                if(hold_time > 150)
                {
                    ...
                    {
                        back_home_time++;
                        std::cout<<"back_home_time:"<<back_home_time<<endl;
                    }
                }
        ...
        // total_vel.data = sqrt(x.out() * x.out() + y.out() * y.out());//这样调参会变得非常困难，并且难以分析
        ...
        // std::cout<<"cos(cur_yaw.data):"<< std::cos(cur_yaw.data)<<"   "<<"sin(cur_yaw.data):"<<sin(cur_yaw.data)<<endl;
        float64 vx = kx * x.out() * std::cos(cur_yaw.data) +
                     ky * y.out() * std::sin(cur_yaw.data);//*360/(2*pi)??????//经过测试，不需要，似乎是自动这么做了
        float64 vy = ky * y.out() * std::sin(cur_yaw.data)+
                     kx * x.out() * std::cos(cur_yaw.data);
