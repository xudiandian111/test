...
void local_yaw_cb(const mavros_msgs::GlobalPositionTarget::ConstPtr& msg) 
{   
    cur_yaw.data = msg->yaw;
...
//         yaw.init(0.0 - yaw.read_beginpose()+3.1415/4.0, 2.0, 0, 0);// 暂时设定yaw的目标角度120度
...
{   
   ...
   {    
        ...
        x.init(x.read_beginpose() + 0.0, 2.0, 0, 0.0);
        y.init(y.read_beginpose() + 0.0, 2.0, 0, 0.0);
        z.init(z.read_beginpose() + 2.0, 1, 0, 0.0);
        flag_getbegin_pos = 1;
    }
    ...
    // double AllSetPositionX[] = {0.0, 0.0, 0.0, 0.0};
    // double AllSetPositionY[] = {0.0, 0.0, 0.0, 0.0};
    // cout<<"z_curpose:"<<z.read_curpose()<<"     "<<"z_beginpose:"<<z.read_beginpose()<<endl;
    ...
    {     
        ...
        // total_vel.data = std::sqrt(x.out() * x.out() + y.out() * y.out());//这样调参会变得非常困难，并且难以分析
        //BODY_NED 下需要将速度向X,Y向分解

        // int kx = x.out()/(std::fabs(x.out()));//暂未考虑除零的情况
        // int ky = y.out()/(std::fabs(y.out()));

        // float64 vx = kx * (total_vel.data) * std::cos(cur_yaw.data);
        // float64 vy = ky * (total_vel.data) * std::sin(cur_yaw.data);

        // float64 vx = x.out()* std::cos(cur_yaw.data) + y.out()*std::sin(cur_yaw.data);
        // float64 vy = x.out()* std::sin(cur_yaw.data) + y.out()*std::cos(cur_yaw.data);写反了
        float64 vx = x.out()* std::cos(cur_yaw.data) + y.out()*std::sin(cur_yaw.data);
        float64 vy = -x.out()* std::sin(cur_yaw.data) + y.out()*std::cos(cur_yaw.data);
        //可以理解为我赋值得速度，最终会被分解到LOCAL_NED下

        //在BODY_NED的frame下，速度就是，不需要控制yaw转动飞机


        set_LinearVel_XY(vel, vx ,vy);
        // set_LinearVel_XY(vel, x.out(), y.out());

        // set_LinearVel_XY(vel ,-total_vel.data*cos(cur_yaw.data) ,-total_vel.data*sin(cur_yaw.data));
        ...
        // ROS_INFO("x.vel[%lf]       ,y.vel[%lf]", x.out() ,y.out());
        // //测试
        // vel.angular.z = 10;
...
int main(int argc, char **argv)
{
    ...
    ros::ServiceClient arming_client = nh.serviceClient<mavros_msgs::CommandBool>
            ("mavros/cmd/arming");
    ros::Subscriber yaw_sub = nh.subscribe<mavros_msgs::GlobalPositionTarget>
            ("mavros/setpoint_raw/target_global", 10, local_yaw_cb);
   ...
   while(ros::ok() && !current_state.connected)
    ...
    if(z.read_curpose() - z.read_beginpose() > 3.0)
    {
        vel.linear.z = 0;
