...
{   
    ...
    {    
        ...
        // yaw.init(0.0 - cur_yaw.data + 2*pi/3.0, 2.0, 0, 0);//暂时设定120
...
{   
   ...
   {    
        ...
        x.init(0.0, 1.0, 0, 0.0);
        y.init(0.0, 1.0, 0, 0.0);
        z.init(2.0, 2.0, 0, 0.0);
        flag_getbegin_pos = 1;
    }
    ...
    {   
        ...
        x.set_curpose((msg->pose.position.x) * cos(cur_yaw.data) - 
                        (msg->pose.position.y) * sin(cur_yaw.data));
        y.set_curpose(-(msg->pose.position.x) * sin(cur_yaw.data) +
                        (msg->pose.position.y) * cos(cur_yaw.data));
    ...
    {     
        ...
        ROS_INFO("x.out:[%lf],y.out:[%lf]",vx, vy);
        cout<<endl;
