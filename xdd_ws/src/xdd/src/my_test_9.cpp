...
{   
    ...
    {    
            ...
            // yaw.init(0 - cur_yaw.data , 2.0, 0, 0);// 暂时设定120
            yaw.init(0.0 , 2.0, 0, 0);// 暂时设定yaw不变
            // yaw.init(0.0 - yaw.read_beginpose() + 3.1415 / 3.0, 2.0, 0, 0);// 暂时设定yaw的目标角度120度
...
{   
   ...
   {    
        ...
        x.init(x.read_beginpose() + 0.0, 0.5, 0, 0.2);
        y.init(y.read_beginpose() + 0.0, 0.5, 0, 0.2);
        z.init(z.read_beginpose() + 2.0, 2.0, 0, 0.2);
        flag_getbegin_pos = 1;
    }
    ...
    {     
        ...
        if(abs(x.read_err()) < 0.15 && abs(y.read_err()) < 0.15)
        {
             hold_time++;
        }
