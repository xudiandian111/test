...
void cur_z_cb(const sensor_msgs::Range::ConstPtr& msg)
{
    ...
    if(flag_getbegin_z == 0)
    {
    	if(msg->range >= msg->min_range && msg->range <= 1.0)
    	{
        	...
        	z.init(z.read_beginpose()+0.5, 1, 0, 0);
			//z.init(0.50, 2, 0, 0);
			flag_getbegin_z = 1;
...
{   
   ...
   {    
        ...
        x.init(x.read_beginpose() + 0.0, 0.2, 0, 0.0);
        y.init(y.read_beginpose() + 0.0, 0.2, 0, 0.0);
    ...
    {     
        ...
        float64 vx = x.out()* std::cos(cur_yaw.data) + y.out()*std::sin(cur_yaw.data);
        float64 vy = -x.out()* std::sin(cur_yaw.data) + y.out()*std::cos(cur_yaw.data);//一开始少写了一个负hao
